...
public class _Strings_Lcs_L97_InterleavingString {

	public static void main(String[] args) {
		_Strings_Lcs_L97_InterleavingString obj = new _Strings_Lcs_L97_InterleavingString();
		...
		for (int i = 0; i <= m; i++) {
			for (int j = 0; j <= n; j++) {
				if (i == 0 && j == 0) {
					...
					if (s1.charAt(i - 1) == s3.charAt(i + j - 1) && tab[i - 1][j]) {
						tab[i][j] = true;
					}else if (s2.charAt(j - 1) == s3.charAt(i + j - 1) && tab[i][j - 1]) {
						tab[i][j] = true;
					}
				}
			}
