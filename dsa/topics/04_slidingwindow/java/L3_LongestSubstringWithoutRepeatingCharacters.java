public class L3_LongestSubstringWithoutRepeatingCharacters {
	...
	// Idea: Use sliding window. You can store the index i+1 with each repeating
	// char in map so that, j can be jumped to that.
	public int lengthOfLongestSubstring(String s) {
		...
		while (r < n) {
			...
			if (set[ch] != 0) {
				l = Math.max(l, set[ch]); // abba case, it is max with l becuase some other char might have updated l
											// also.
			}
