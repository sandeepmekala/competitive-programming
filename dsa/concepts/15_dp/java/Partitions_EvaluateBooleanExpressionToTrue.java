public class Partitions_EvaluateBooleanExpressionToTrue {
    ...
    public long evaluateexp2(String exp) {
        int n = exp.length();
        return evaluateexp2(0, n - 1, 1, exp);
    }

    int mod = 1000000007;
    public long evaluateexp2(int i, int j, int isTrue, String exp) {
        // Base case 1:
        if (i > j)
            return 0;
        // Base case 2:
        ...
        long ways = 0;
        for (int ind = i + 1; ind <= j - 1; ind += 2) {
            long lT = evaluateexp2(i, ind - 1, 1, exp);
            long lF = evaluateexp2(i, ind - 1, 0, exp);
            long rT = evaluateexp2(ind + 1, j, 1, exp);
            long rF = evaluateexp2(ind + 1, j, 0, exp);
