...
public class Subsequence_01Knapsack {
	...
	public static void main(String[] args) {
		...
		int n = 4, wt = 10;
		System.out.println(obj.knapsack(weights, values, n-1, wt));
		System.out.println(obj.knapsack(weights, values, wt));	// n, W
	}
	...
	// Time: O(n*Wmax)
    // Space: O(n*Wmax)
	public int knapsack(int[] weight, int[] value, int ind, int wmax) {
		if (ind == 0){
			if(weight[0] <= wmax)
				return value[0];
			return 0;
	...
	public int knapsack(int[] weight, int[] value, int wmax) {
		int n = weight.length;
