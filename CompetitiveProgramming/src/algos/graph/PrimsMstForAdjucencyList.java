...
public class PrimsMstForAdjucencyList {
	class AdjListNode {
	...
	void addEdge(Graph graph, int src, int dest, int weight)
	{
		...
		AdjListNode node0 = new AdjListNode(dest, weight);
		AdjListNode node = new AdjListNode(src, weight);
		graph.adj[src].addLast(node0);
		graph.adj[dest].addLast(node);
	}

	// O(E+V)*O(LogV) -> O(ElogV) as V=O(E)
	void primsMst(Graph graph)
	{
		...
		while (!queue.isEmpty()) {
			...
			for (AdjListNode iterator : graph.adj[node0.vertex]) {

				// If V is in queue
				if (mstset[iterator.vertix] == false) {
					// If the key value of the adjacent vertex is
					// more than the extracted key
					// update the key value of adjacent vertex
					// to update first remove and add the updated vertex
					if (e[iterator.vertix].key > iterator.weight) {
						queue.remove(e[iterator.vertix]);
						e[iterator.vertix].key = iterator.weight;
						queue.add(e[iterator.vertix]);
						parent[iterator.vertix] = node0.vertex;
					}
				}
			}
		}
	...
	public static void main(String[] args)
	{
		...
		PrimsMstForAdjucencyList e = new PrimsMstForAdjucencyList();
