...
public class MatrixMaxRectangleOfAll1s {
	...
	public static void main(String[] args) {
		...
		System.out.println(maxRecArea(matrix, matrix.length, matrix[0].length));
	}
	
	//idea is to use max rec area in histogram logic to find the max area of 1's
	//for the subsequence rows cumulatively add rows so that we get bigger histograms by consecutive 1's in each column
	//time = O(rows*cols)
	//space = O(cols) 
	private static int maxRecArea(int[][] matrix, int m, int n) {
		int globalMax = 0;
