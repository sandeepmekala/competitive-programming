...
public class Algo052_LongestCommonSubsequence {
	
	private static int mid_results[][] = new int[4][5];
	public static void main(String[] args) {
		Algo052_LongestCommonSubsequence obj = new Algo052_LongestCommonSubsequence();
	...
	private int lcsBottomUp(String p, String q, int m, int n) {
		int mem[][] = new int[m][n];
		for(int i=0; i<p.length(); i++) {
			for(int j=0; j<q.length(); j++) {
				if(p.charAt(i) == q.charAt(j)) {		//matching
					if(i == 0 || j == 0) {
						mem[i][j] = 1;
					}else {
