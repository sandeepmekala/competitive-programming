...
public class Grid_MultiEnd_L1463_CherryPickupII {
    public static void main(String args[]){
        Grid_MultiEnd_L1463_CherryPickupII obj = new Grid_MultiEnd_L1463_CherryPickupII();
    ...
    // Idea: Idea is same like fixed start to variable ends. But there are 2 fixed positions and when to overlap, those cells should be counted only once.
    ...
    public int cherryPickupRec(int[][] grid) {
        int m=grid.length, n=grid[0].length;
        return cherryPickupRec(grid, 0, 0, n-1, m, n);
    }

    private int cherryPickupRec(int[][] grid, int i, int j1, int j2, int m, int n) {
        ...
        for(int dj1=-1; dj1<=1; dj1++){
            for(int dj2=-1; dj2<=1; dj2++){
                ...
                value += cherryPickupRec(grid, i+1, j1+dj1, j2+dj2, m, n);
                max = Math.max(max, value);
            }
        }
        return max;
